	...
	scramble = {
		...
		start: function() {
			if (!scramble.running) {
				...
				bot.sendChat("Unscramble the word: " + scramble.shuffle(scramble.answer.toUpperCase()));
				scramble.running = true;
				scramble.timeout = setTimeout(function() {
					scramble.running = false;
					bot.sendChat("Scramble ended!");
		...
		checkAnswer: function(data) {
			if (data.message.toLowerCase() == scramble.answer) {
				...
				bot.sendChat("@" + data.from.username + " correct! The word was " + scramble.answer.toUpperCase());
